...
def apply_pro_style():
    st.markdown("""
        ...
        .viewport-container {
            ...
            width: 100%; min-height: 85vh; margin-top: 10px;
        }
        .media-preview {
            max-width: 95% !important; 
            max-height: 85vh !important; /* Increased from 60vh to 85vh */
        ...
        with cf:
            if st.button(f"📄 {name}", key=f"file_{pid}", use_container_width=True):
                with st.spinner("Loading..."):
                    ...
                    st.session_state.current_format = f.get('format', 'bin')
...
if st.session_state.file_data:
    ...
    with d_col:
        # FIX: Open Cloudinary URL directly in a new tab
        dl_url = st.session_state.current_url
        st.markdown(
            f'<a href="{dl_url}" target="_blank" class="custom-download-btn">Download</a>',
            unsafe_allow_html=True
        )
